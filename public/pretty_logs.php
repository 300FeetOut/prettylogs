...
class PrettyLogs {
    ...
    private function __construct($auth = '', $project_name = '', $ignore_https_errors, $url) {
        $this->auth = $auth;
    ...
    static function init($auth = '', $project_name = '', $ignore_https_errors = false, $url = null) {
        if (!$url)
        ...
        $new = new PrettyLogs($auth, $project_name, $ignore_https_errors, $url);
        return $new;
    }
    ...
    public function plog() {
        ...
        $caller = @$bt[0]; // We want to know where the call came from!
        $common_log_data = array(
            'referrer' => self::current_page_url(),
            'level' => 'info',
            'stack_trace' => self::prep_backtrace($bt),
            'context' => self::get_code(@$caller['file'], @$caller['line'])
        );
